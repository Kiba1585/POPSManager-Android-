...
using POPSManager.Core.Commands;
...
public class HomeViewModel : BindableObject
{
    ...
    public HomeViewModel(IPathsService paths, ILoggingService log, INotificationService notify,
                          ConverterService converter, GameProcessor processor,
                          SettingsService settings, LocalizationService loc)
    {
        _paths = paths;
        _log = log;
        _notify = notify;
    ...
    {
        // Abrirá la página de conversión (próximo paso)
    }

    private async Task OpenProcessPops()
    {
        // Abrirá la página de procesamiento (próximo paso)
    }

    private void OpenRootFolder()
    {
        // Intentar abrir carpeta raíz (depende de permisos Android)
    }

    private void OpenElfFolder()
    {
        // Intentar abrir carpeta ELF
    }
}