    ...
    public class GameProcessingService
    {
        ...
        public async Task<string> RenameAllAsync(bool cheatWidescreen, bool cheatNoPal, bool cheatFixSound, bool cheatFixGraphics)
        {
            ...
            foreach (var g in ps1.ToList())
            {
                try
                {
                    ...
                    if (string.Equals(g.FilePath, newPath, StringComparison.OrdinalIgnoreCase)) { skipped++; continue; }
        ...
        private void CreateMultidiscStructure(ObservableCollection<GameItem> ps1Games)
        {
            var allVcds = Directory.GetFiles(_paths.PopsFolder, "*.VCD", SearchOption.TopDirectoryOnly)
                .Concat(Directory.GetFiles(_paths.PopsFolder, "*.vcd", SearchOption.TopDirectoryOnly)).ToList();

            var groups = new Dictionary<string, List<string>>(StringComparer.OrdinalIgnoreCase);
            foreach (var f in allVcds)
            {
                string n = Path.GetFileNameWithoutExtension(f);
                n = Regex.Replace(n, @"\s*\((CD|Disc)\s*\d\)", "", RegexOptions.IgnoreCase);
                n = Regex.Replace(n, @"\s*(CD|Disc)\s*\d", "", RegexOptions.IgnoreCase).Trim();
                if (!groups.ContainsKey(n)) groups[n] = new List<string>();
                groups[n].Add(f);
            }
            ...
            foreach (var grp in groups.Where(g => g.Value.Count > 1))
            {
                ...
                string common = Path.Combine(_paths.PopsFolder, grp.Key);
                Directory.CreateDirectory(common);
                string txt = Path.Combine(common, "DISCS.TXT");
                if (!File.Exists(txt)) File.WriteAllLines(txt, files.Select(Path.GetFileName));

                for (int i = 0; i < files.Count; i++)
                {
                    var g = ps1Games.FirstOrDefault(x => x.FilePath.Equals(files[i], StringComparison.OrdinalIgnoreCase));
                    if (g != null) { g.IsMultiDisc = true; g.DiscNumber = i + 1; g.GameFolder = common; }
                }
            }
        }
    }
