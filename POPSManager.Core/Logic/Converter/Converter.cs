...
namespace POPSManager.Core.Logic
{
    public class Converter
    {
        ...
        public Converter(Action<int> updateProgress, Action<string> updateSpinner, Action<string> log,
                         Action<UiNotification> notify, PathsService paths, LocalizationService loc)
        {
        ...
        public void ConvertSingle(string inputPath)
        {
            ...
            if (inputPath.EndsWith(".cue", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (cue == null)
                {
                    _notify(new UiNotification { Type = NotificationType.Error, Message = string.Format(_loc.GetString("Converter_InvalidCue"), fileName) });
                    return;
                }
            ...
            {
                _notify(new UiNotification { Type = NotificationType.Error, Message = string.Format(_loc.GetString("Converter_UnknownSectorFormat"), fileName) });
                return;
            }

            VcdHeader.Write(output, fileName, input.Length, _log);
            SectorConverter.Convert(input, output, mode, _updateProgress, _log);
            _notify(new UiNotification { Type = NotificationType.Success, Message = string.Format(_loc.GetString("Converter_VcdGenerated"), fileName) });
        }

        private static bool IsPs2Iso(string path)
        {
            string name = Path.GetFileName(path).ToUpperInvariant();
            string[] ps2Patterns = { "SLUS_2", "SLUS_3", "SCUS_9", "SLES_5", "SLES_6", "SCES_5", "SCES_6" };
