...
namespace POPSManager.Core.Logic
{
    public static class GameIdDetector
    {
            ...
            try
            {
                ...
                if (rootLba > 0)
                {
                    ...
                    if (sys.lba > 0)
                    {
                        ...
                        if (id != null) return Normalize(id);
                    }
                }

                fs.Seek(0, SeekOrigin.Begin);
                var deep = ScanForId(fs, 2 * 1024 * 1024);
                if (deep != null) return Normalize(deep);
            }
            catch { }
        ...
        private static string? ScanForId(FileStream fs, int bytes)
        {
            ...
            var m = Ps1Regex.Match(text);
            if (m.Success) return $"{m.Groups[1].Value}_{m.Groups[2].Value}.{m.Groups[3].Value}";
            m = Ps2Regex.Match(text);
            return m.Success ? $"{m.Groups[1].Value}_{m.Groups[2].Value}" : null;
        }
        ...
        public static string DetectFromName(string name)
        {
            ...
            return m.Success ? $"{m.Groups[1].Value}_{m.Groups[2].Value}" : "";
        }

        private static string Normalize(string? id)
        {
            if (string.IsNullOrWhiteSpace(id)) return "";
            return id.ToUpperInvariant().Replace("-", "_").Replace(" ", "_").Replace(".", "_");
        }
        ...
        public static bool IsPalRegion(string gameId)
        {
            ...
            return gameId.StartsWith("SLES", StringComparison.Ordinal) || gameId.StartsWith("SCES", StringComparison.Ordinal);
        }

        public static bool RequiresPal60(string gameId)
        {
            if (string.IsNullOrWhiteSpace(gameId)) return false;
            string[] pal60Ids = { "SLES_01514", "SLES_01370", "SLES_02080", "SLES_02965", "SCES_01237", "SCES_02105", "SCES_02104", "SCES_02835" };
            foreach (var id in pal60Ids)
                if (gameId.StartsWith(id, StringComparison.OrdinalIgnoreCase)) return true;
            return false;
        }
    }
}